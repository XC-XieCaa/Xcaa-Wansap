(async () => {
     switch(true) {
         case /^(menu|help)$/i.test(command): {
             ...
             conn.sendMessage(from, { contentText: menu(m, body, pushname).trim(), footerText: '© FxSx', buttons: [{ buttonId: `${prefix}owner`, buttonText: { displayText: 'OWNER' }, type: 1 },{ buttonId: `${prefix}tos`, buttonText: { displayText: 'TUTORIAL' }, type: 1 }], headerType: 'IMAGE', imageMessage: prep }, 'buttonsMessage', { quoted: m })
             break
         }
