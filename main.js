...
async function start(session) {
    ...
    console.log(color('[ XDT ]', 'yellow'), color('Loading...'))
    conn.on('qr', () => {
        console.log(color('[ ! ]', 'yellow'), color('Scan QR Code'))
    })
    if (fs.existsSync(session)) conn.loadAuthInfo(session)
    conn.on('connecting', () => {
        console.log(color('[ XDT ]', 'yellow'), color('Connecting...'))
    })
    conn.on('open', () => {
        console.log(color('[ XDT ]', 'yellow'), color('Connected!!'))
    })
    conn.connect().then(() => {
